...
# tmux servers are often long-lived, so their PATH can predate a newly installed
# assistant. Resolve the executable here and pass tmux an absolute path.
PATH="$HOME/.local/bin:$PATH"
assistant_cmd=$(command -v "$LLM_ASSISTANT" 2>/dev/null)
if [[ -z "$assistant_cmd" ]]; then
  tmux display-message -d 5000 "Unable to find '$LLM_ASSISTANT' in PATH"
  exit 127
fi
printf -v assistant_shell_cmd 'exec %q' "$assistant_cmd"
...
if [[ "$session_name" == popup-* ]]; then
  if [[ "$window_name" == "$LLM_ASSISTANT" ]]; then
    ...
    tmux select-window -t "$session_name:$LLM_ASSISTANT" 2>/dev/null ||
      tmux new-window -t "$session_name" -n "$LLM_ASSISTANT" -c "$pane_path" "$assistant_shell_cmd"
  fi
else
  ...
  tmux has-session -t "$POPUP_NAME" 2>/dev/null ||
    tmux new-session -d -s "$POPUP_NAME" -c "$pane_path" -n "$LLM_ASSISTANT" "$assistant_shell_cmd"

  tmux select-window -t "$POPUP_NAME:$LLM_ASSISTANT" 2>/dev/null ||
    tmux new-window -t "$POPUP_NAME" -n "$LLM_ASSISTANT" -c "$pane_path" "$assistant_shell_cmd"
